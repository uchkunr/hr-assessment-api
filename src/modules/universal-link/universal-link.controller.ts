  ...
  createRoutes() {
    return new Elysia({ prefix: '/links' })
      .get(
        ...
        async ({ status }) => {
          try {
            ...
            return status(500, {
              message: err instanceof Error ? err.message : 'Failed to fetch universal link',
            });
          }
        },
        {
      ...
      .get(
        ...
        async ({ params, status }) => {
          try {
            const assignment = await this.assignmentService.findByTestId(params.testId);
            if (!assignment) {
            ...
            return status(500, {
              message: err instanceof Error ? err.message : 'Failed to fetch assignment',
            });
          }
        },
        {
