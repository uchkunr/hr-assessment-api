...
export class QuestionService {
  ...
  async findById(testId: string, assignmentId: string): Promise<QuestionBO[]> {
    ...
    const selectedMap = new Map(selected.map((s) => [s.questionId, s.answerId]));
  ...
  async update(questionId: string, data: UpdateQuestionRequestBO): Promise<QuestionBO> {
  ...
  private async initProgress(assignmentId: string): Promise<void> {
    ...
    const assignmentTest = await QuestionService.prisma.assignmentTest.findUnique({
  ...
  private async calculateProgress(prisma: any, assignmentId: string): Promise<void> {
