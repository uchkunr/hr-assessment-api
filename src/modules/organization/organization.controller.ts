  ...
  createRoutes() {
    return new Elysia({ prefix: '/organizations' })
      ...
      .get(
        ...
        async ({ query, status }) => {
          try {
            ...
            const result = await this.organizationService.findAll(page, limit, search);
            return {
      ...
      .get(
        ...
        async ({ params, status }) => {
          try {
            const organization = await this.organizationService.findById(params.id);
            if (!organization) {
            ...
            return status(500, {
              message: err instanceof Error ? err.message : 'Failed to fetch organization',
            });
          }
        },
        {
